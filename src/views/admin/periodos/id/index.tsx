...
export default function AdminPeriodoIDView() {
    ...
    const [toastMessageSeverity, setToastMessageSeverity] =
        React.useState<AlertColor>("success");
    const [titleDialogConfirmation, setTitleDialogConfirmation] =
        React.useState("");
    const [messageDialogConfirmation, setMessageDialogConfirmation] =
        React.useState("");
    const { periodos: periodosReducer } = useAppSelector(
        (state: RootState) => state.periodos
    );
    ...
    const [currentOper, setCurrentOper] = React.useState<OPER>(
        OPER.NOT_DEFINED
    );
    ...
    const [periodo, setPeriodo] = React.useState<Periodo>(
        useAppSelector((state: RootState) =>
            selectPeriodoById(state, parseInt(id || "0"))
        )
    );
                ...
                clases.map((clase_) => {
                    if (clase_.id === clase.id) {
                        dispatch(
                            updateClase({
                                ...clase_,
                                imagen: `${config.baseUrl}${config.thumbnailPath}/${data.data[0].filename}`,
                            })
                        );
                        return {
                            ...clase_,
    ...
    const handleOpenDialogConfirmation = (
        ...
        setMessageDialogConfirmation(
            `¿Está seguro que desea eliminar la clase "${clase.titulo}"?`
        );
    ...
    const handleClick = (event: React.MouseEvent<unknown>, clase: Clase) => {
        navigate(
            `/admin/clientes/${clienteId}/periodos/${id}/clases/${clase.id}`,
            { replace: false }
        );
    };
        ...
        <Box sx={{ width: "100%" }}>
            ...
            <Dialog open={openDialogClase} onClose={handleCloseDialogClase}>
                <Box component="form" onSubmit={handleSubmitClase}>
                    ...
                    <DialogActions>
                        <Button onClick={handleCloseDialogClase}>
                            Cancelar
                        </Button>
            ...
            >
                ...
                >
                    ...
                    <Button
                        ...
                        aria-controls={
                            openMenuClase ? "demo-customized-menu" : undefined
                        }
                        aria-haspopup="true"
                    ...
                    >
                        <MenuItem
                            onClick={() =>
                                handleOpenDialogClase(TIPO_CLASE.INDUCCION)
                            }
                            disableRipple
                        >
                            <MenuBookIcon />
                        ...
                        <MenuItem
                            onClick={() =>
                                handleOpenDialogClase(TIPO_CLASE.CAPACITACION)
                            }
                            disableRipple
                        >
                            <ConstructionIcon />
                        ...
                        <MenuItem
                            onClick={() =>
                                handleOpenDialogClase(TIPO_CLASE.DOCUMENTACION)
                            }
                            disableRipple
                        >
                            <PictureAsPdfIcon />
            ...
            <ConfirmationDialog
                ...
                title={titleDialogConfirmation}
                message={messageDialogConfirmation}
            />

            <Snackbar
                open={openToastResponse}
                autoHideDuration={6000}
                onClose={handleCloseToastResponse}
            >
                <Alert
                    onClose={handleCloseToastResponse}
