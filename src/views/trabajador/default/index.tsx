...
export default function ViewTrabajadorDefault(){
    ...
    const inducciones = clases.filter(clase => clase.tipo === TIPO_CLASE.INDUCCION);
    const capacitaciones = clases.filter(clase => clase.tipo === TIPO_CLASE.CAPACITACION);
    const documentaciones = clases.filter(clase => clase.tipo === TIPO_CLASE.DOCUMENTACION);


    const renderClases = (clases: Clase[]) => {
        return clases.map(classWorker => {
            const disponible = moment().isAfter(moment(classWorker.fecha_inicio), 'minutes') &&
                moment().isBefore(moment(classWorker.fecha_fin), 'minutes');
            return (
                <Grid display={"flex"} alignItems={"stretch"} item key={classWorker.id} xs={12} sm={6} md={4}>
                    <CardClassWorker classWorker={classWorker} disponible={disponible} />
                </Grid>
            );
        });
    };
